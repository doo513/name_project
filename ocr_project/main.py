...
class MainApp:
    ...
    def _start_ocr_worker(self, session_id: int, region: Region, image) -> None:
        ...
        self._set_capture_status(f"Running OCR on the selected region... source OCR language: {ocr_languages[0]}")
        if self.capture_monitor is not None:
            self.capture_monitor.set_status(f"Running OCR... [{ocr_languages[0]}]")
